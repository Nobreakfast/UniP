...
from model.backbone.attention_modules.eca import eca_block
from model.backbone.radar.RadarEncoder import RCNet
from model.neck.ghostdualfpn import GhostDualFPN
from model.neck.cspdualfpn import CSPDualFPN
...
class IREncoder(nn.Module):
    def __init__(
        ...
        # ================================= stage 3 ===================================== #
        # ------------------------ 分别度量每个分支内的通道重要程度 --------------------- #
        self.channel_attn_stage3 = nn.ModuleList(
            [
                eca_block(channel=image_encoder_width[phi][1])
                if i == 0
        ...
        # ================================= stage 4 ===================================== #
        self.channel_attn_stage4 = nn.ModuleList(
            [
                eca_block(channel=image_encoder_width[phi][2])
                if i == 0
        ...
        # ================================= stage 5 ===================================== #
        self.channel_attn_stage5 = nn.ModuleList(
            [
                eca_block(channel=image_encoder_width[phi][3])
                if i == 0
    ...
    def forward(self, x, x_radar):
        ...
        (
            se_seg_output,
            lane_seg_output,
            (map_stage5, map_stage4, map_stage3),
        ) = image_features

        radar_features = self.radar_encoder(x_radar)
        radar_stage3, radar_stage4, radar_stage5 = radar_features

        p3_ir_fuse = torch.cat(
            [
                self.channel_attn_stage3[0](map_stage3),
                self.channel_attn_stage3[1](radar_stage3),
            ],
            dim=1,
        )
        p3_fuse_out = self.act_stage3(self.norm_stage3(p3_ir_fuse))

        p4_ir_fuse = torch.cat(
            [
                self.channel_attn_stage4[0](map_stage4),
                self.channel_attn_stage4[1](radar_stage4),
            ],
            dim=1,
        )
        p4_fuse_out = self.act_stage4(self.norm_stage4(p4_ir_fuse))

        p5_ir_fuse = torch.cat(
            [
                self.channel_attn_stage5[0](map_stage5),
                self.channel_attn_stage5[1](radar_stage5),
            ],
            dim=1,
        )
