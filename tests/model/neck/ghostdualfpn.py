...
from model.backbone.vision.ImageEncoder import *
from model.neck.spp import *
from model.backbone.conv_utils.normal_conv import *
from model.backbone.conv_utils.ghost_conv import *
from model.backbone.attention_modules.shuffle_attention import *
from model.backbone.vision.mobilevit_modules.mobilevit import (
    mobilevit_xxs,
    mobilevit_xs,
    mobilevit_s,
)
from model.backbone.vision.edgenext_modules.model import (
    edgenext_xx_small,
    edgenext_x_small,
    edgenext_small,
)
from model.backbone.vision.edgevit_modules.edgevit import (
    edgevit_xxs,
    edgevit_xs,
    edgevit_s,
)
from model.backbone.vision.repvit_modules.repvit import repvit_m1, repvit_m2, repvit_m3
from model.backbone.vision.poolformer_modules.poolformer import (
    poolformer_S0,
    poolformer_S1,
    poolformer_S2,
)
...
class GhostDualFPN(nn.Module):
    def __init__(
        self, num_class_seg, resolution=416, phi="S0", use_spp=True, backbone="ef"
    ):
        ...
        if self.num_class_seg > 31:
            assert (
                "class number of semantic segmentation must be smaller than 32 (<=31)"
            )
        ...
        # 176, 16, 16 -> 192, 32, 32
        self.upsample_5_to_4 = Upsample(
            in_channels=self.channel_widths[-1], out_channels=self.channel_widths[-2]
        )
        self.ghost_5_to_4 = GhostBottleneck(
            in_chs=self.channel_widths[-2] * 2,
            mid_chs=self.channel_widths[-2] * 2,
            out_chs=self.channel_widths[-2],
        )

        # 192, 32, 32 -> 96, 64, 64
        self.upsample_4_to_3 = Upsample(
            in_channels=self.channel_widths[-2], out_channels=self.channel_widths[-3]
        )
        self.ghost_4_to_3 = GhostBottleneck(
            in_chs=self.channel_widths[-3] * 2,
            mid_chs=self.channel_widths[-3] * 2,
        ...
        self.stage_3_semantic_seg = ShuffleAttention(
            channel=self.channel_widths[-3], G=4
        )

        # ======================================= WaterLine Segmentation ========================================= #
        # lane-segmentation 96, 64, 64 -> 48, 128, 128
        self.lane_seg_3_to_2 = Upsample(
            in_channels=self.channel_widths[-3], out_channels=self.channel_widths[-3]
        )
        self.lane_seg_ghost_3_to_2 = GhostModule(
            inp=self.channel_widths[-3], oup=self.channel_widths[-3]
        )

        # lane-segmentation 48, 128, 128 -> 32, 256, 256
        self.lane_seg_2_to_1 = Upsample(
            in_channels=self.channel_widths[-3], out_channels=self.channel_widths[-4]
        )
        self.lane_seg_ghost_2_to_1 = GhostModule(
            inp=self.channel_widths[-4], oup=self.channel_widths[-4]
        )

        # lane-segmentation 32, 256, 256 -> 32, 512, 512
        self.lane_seg_1_to_0 = Upsample(
            in_channels=self.channel_widths[-4], out_channels=self.channel_widths[-4]
        )
        self.lane_seg_ghost_1_to_0 = GhostModule(
            inp=self.channel_widths[-4], oup=self.channel_widths[-4]
        )
        ...
        self.se_seg_3_to_2 = Upsample(
            in_channels=self.channel_widths[-3], out_channels=self.channel_widths[-3]
        )
        self.se_seg_ghost_3_to_2 = GhostModule(
            inp=self.channel_widths[-3], oup=self.channel_widths[-3]
        )

        # semantic-segmentation 48, 128, 128 -> 32, 256, 256
        self.se_seg_2_to_1 = Upsample(
            in_channels=self.channel_widths[-3], out_channels=self.channel_widths[-4]
        )
        self.se_seg_ghost_2_to_1 = GhostModule(
            inp=self.channel_widths[-4], oup=self.channel_widths[-4]
        )

        # semantic-segmentation 32, 256, 256 -> 32, 512, 512
        self.se_seg_1_to_0 = Upsample(
            in_channels=self.channel_widths[-4], out_channels=self.channel_widths[-4]
        )
        self.se_seg_ghost_1_to_0 = GhostModule(
            inp=self.channel_widths[-4], oup=self.channel_widths[-4]
        )

        # semantic-segmentation 32, 512, 512 -> num_class_seg, 512, 512
        self.se_seg_head = GhostModule(
            inp=self.channel_widths[-4], oup=self.num_class_seg
        )
        # ======================================================================================================== #
    ...
    def forward(self, x):
        ...
        return (
            se_seg_output,
            lane_seg_output,
            (fpn_stage5 + map_stage5, fpn_stage4 + map_stage4, fpn_stage3 + map_stage3),
        )
